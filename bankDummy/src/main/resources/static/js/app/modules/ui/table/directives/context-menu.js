...
angular
   ...
   .directive('contextMenu', ["$parse", "$q", "$sce", function ($parse, $q, custom, $sce) {
       ...
       var processItem = function ($scope, event, model, item, $ul, $li, $promises, $q, $, level) {
           ...
           "use strict";
           // nestedMenu is either an Array or a Promise that will return that
           // array.
           ...
           // if html property is not defined, fallback to text, otherwise use
           // default text
       ...
       var handlePromises = function ($ul, level, event, $promises) {
           // / <summary>
           // / calculate if drop down menu would go out of screen at left or
           // bottom
           // / calculation need to be done after element has been added (and
           // all
           // texts are set; thus thepromises)
           ...
           $q.all($promises).then(function () {
               ...
               if (topCoordinate > menuHeight && winHeight - topCoordinate < menuHeight) {
                   ...
                   // If it really can't fit, reset the height of the menu to one
                   // that
                   // will fit
                   angular.element($ul[0]).css({ "height": winHeight - 5, "overflow-y": "scroll" });
                   // ...then set the topCoordinate height to 0 so the menu starts
                   // from
                   // the top
                   topCoordinate = 0;
       ...
       var registerEnabledEvents = function ($scope, enabled, item, $ul, $li, nestedMenu, model, text, event, $, level) {
           // / <summary>If item is enabled, register various mouse
           // events.</summary>
           if (enabled) {
               var openNestedMenu = function ($event) {
                   ...
                   /*
                    * At this point, nestedMenu can only either be an Array or a
                    * promise. Regardless, passing them to when makes the
       ...
       var renderContextMenu = function ($scope, event, options, model, level, customClass) {
           ...
           var $ul = $('<ul>');
           $ul.addClass('dropdown-menu table-menu basic-btn');
           $ul.attr({ 'role': 'menu' });
           $ul.css({
               display: 'block',
               position: 'absolute',
       ...
       return function ($scope, element, attrs) {
           ...
           element.on(openMenuEvent, function (event) {
               ...
               // Don't show context menu if on touch device and element is
               // draggable
