package gec.scf.dummy.config.authentication;
...
public class UniqueAuthenticationKeyGenerator implements AuthenticationKeyGenerator {
	...
	public String extractKey(OAuth2Authentication authentication) {
		...
		if (authorizationRequest.getScope() != null) {
			values.put(SCOPE, OAuth2Utils
					.formatParameterList(new TreeSet<>(authorizationRequest.getScope())));
		}
	...
	protected String generateKey(Map<String, String> values) {
		...
		try {
			...
			throw new IllegalStateException(
					"UTF-8 encoding not available.  Fatal (should be in the JDK).", uee);
		}
	}
}